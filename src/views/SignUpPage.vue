          ...
          <div class="divide-y divide-gray-200 dark:divide-gray-700">
            <form @submit.prevent="handleSignup">
              <div class="py-8 text-base leading-6 space-y-4 text-gray-700 dark:text-gray-300 sm:text-lg sm:leading-7">
                ...
                <p v-if="invalidEmail" class="text-sm text-red-500">Please enter a valid email address.</p>
...
//Define the regex pattern for a email format 
const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/

const passwordMismatch = computed(() => form.value.password !== form.value.confirmPassword)
const invalidEmail = computed(() => !emailPattern.test(form.value.email))
